...
8. Do not disable TLS verification.
9. Normal logs must not dump full UDIDs, pair records, credentials or other secrets.
10. Prefer `pymobiledevice3` documented/public library APIs. Do not import `pymobiledevice3.cli.*` internals into application code.
11. Prefer one coherent asyncio runtime. Do not introduce unmanaged background threads when asyncio tasks can express the lifecycle.
12. Avoid dependency additions unless they remove more complexity than they add.
...
The modernization target is Python 3.14 on development machines unless a platform-specific blocker is demonstrated. `pymobiledevice3` must be pinned through the project lockfile, initially against the current audited major version rather than floated. Packaging must also be pinned.
...
- targeted tests pass;
- the relevant regression case in `docs/TEST_MATRIX.md` is covered or explicitly marked hardware-only;
